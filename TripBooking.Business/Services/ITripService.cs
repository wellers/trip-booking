namespace TripBooking.Business.Services
{
	...
	{
		Task<Shared.Response.Trip?> CreateTripAsync(Shared.Dtos.Trip trip, CancellationToken token);
		Task<bool> UpdateTripAsync(int id, Shared.Dtos.Trip trip, CancellationToken token);
		Task<bool> PatchTripAsync(int id, Shared.Dtos.Trip trip, CancellationToken token);
